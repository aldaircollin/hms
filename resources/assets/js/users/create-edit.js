...
$(document).ready(function () {
    ...
    $(document).ready(function () {
        $('#role, #doctorDepartment').select2({
            width: '100%',
        });
        $('#role').on('change', function () {
            let role = $(this).val();
            if (typeof doctorRole !== 'undefined') {
                if (role === doctorRole) {
                    $('#doctorDepartment').prop('required', true);
                    $('.doctor-department').removeClass('d-none');
                } else {
                    $('#doctorDepartment').prop('required', false);
                    $('.doctor-department').addClass('d-none');
                }
            }
        });
    });
